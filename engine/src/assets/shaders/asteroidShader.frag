...
const Layer LAYERS[N_LAYERS] = Layer[](
  Layer(vec3(0.05, 0.00, 0.30),0.0,0.20),   // Oceano profundo (roxo escuro)
  Layer(vec3(0.20, 0.00, 0.50),0.20,0.30),  // Oceano raso (violeta intenso)
  Layer(vec3(0.95, 0.20, 0.85),0.30,0.35),  // Costas / praia alien (rosa-magenta neon)
  Layer(vec3(0.20, 0.95, 0.60),0.35,0.45),  // Vegetação baixa (verde água fluorescente)
  Layer(vec3(0.00, 0.75, 0.90),0.45,0.55),  // Vegetação média (ciano vivo)
  Layer(vec3(0.10, 0.40, 0.70),0.55,0.60),  // Vegetação alta (azul sci-fi)
  Layer(vec3(0.70, 0.30, 0.80),0.60,0.70),  // Montanhas baixas (roxo lilás)
  Layer(vec3(0.50, 0.10, 0.60),0.70,0.80),  // Montanhas médias (púrpura profundo)
  Layer(vec3(0.80, 0.80, 0.25),0.80,0.90),  // Montanhas altas (amarelo-esverdeado alien)
  Layer(vec3(1.00, 0.95, 0.95),0.90,1.00)   // Topo / neve alien (branco rosado)
);

// --- Cores de nuvem ---
// --- Cores de nuvem estilizadas ---
const int N_CLOUD_COLORS=4;
const vec3 CLOUD_COLORS[N_CLOUD_COLORS]=vec3[](
  vec3(0.25, 0.45, 0.85), // azul médio
  vec3(0.55, 0.75, 0.95), // azul claro
  vec3(0.85, 0.90, 1.00), // quase branco azulado
...
if (r > 1.0) discard;
  ...
  // suavizar transições entre layers
  vec3 color = LAYERS[0].color;
  for (int i=0; i<N_LAYERS-1; i++) {
    float t = smoothstep(LAYERS[i].widthMax, LAYERS[i+1].widthMin, elev);
    color = mix(color, LAYERS[i+1].color, t);
  }
